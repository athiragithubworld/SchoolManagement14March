              ...
              <div className="overflow-hidden">
                ...
                <table className="w-full flex flex-col gap-[10px]">
                    ...
                    {/* <div className={classes.scrolldata}> */}
                        ...
                        {Array.isArray(sortedFacultySalaryDetails)  &&sortedFacultySalaryDetails.map((faculty, index) => (
                          <tr className="w-full h-[52px] rounded-[14px] flex border-[1px] items-center justify-between p-3 shadow-md bg-white shrink-0"
                          key={faculty.id} onClick={() => handleRowClick(faculty)}
                          onDoubleClick={() => handleRowDoubleClick(faculty)}
                          >

                            <td className="w-32 h-fit text-lg text-center font-normal">{index + 1}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.facultyName}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.facultyId}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.bankName}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.branch}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.ifsc}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">{faculty.bankAc}</td>
                            <td className="w-32 h-fit text-lg text-center font-normal">₹{faculty.amount}</td>
